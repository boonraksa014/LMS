...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (!user.active) return res.status(401).json({ error: 'บัญชีนี้ถูกระงับการใช้งาน' });
...
app.post('/api/users', async (req, res) => {
  try {
    const { name, email, password, role = 'learner', group = '', employeeId = '', active = true } = req.body;
    const hash = await bcrypt.hash(password, 10);
    const id = `user_${Date.now()}`;
    const { rows } = await pool.query(
      `INSERT INTO users (id, name, email, password, role, "group", employee_id, active)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
      [id, name, email, hash, role, group, employeeId, active]
    );
...
app.patch('/api/users/:id', async (req, res) => {
  try {
    const { name, email, password, role, group, employeeId, active } = req.body;
    const sets = [];
    const vals = [];
    let i = 1;
    ...
    if (group       !== undefined) { sets.push(`"group"=$${i++}`);     vals.push(group); }
